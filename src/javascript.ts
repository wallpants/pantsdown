   ...
   return `
    ...
    (() => {
        ...
        function setup(el) {
            ...
            const svg = el.querySelector(".mermaid-canvas svg") ?? el.querySelector(":scope > svg");
            if (!svg) return;
        ...
        // so wait for the svg to replace the diagram source. Only a direct-child
        // svg counts: mermaid.run() renders in place via a nested temp div
        // whose skeleton svg must not be grabbed mid-render
